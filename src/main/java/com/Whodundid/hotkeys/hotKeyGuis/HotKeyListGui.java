...
import com.Whodundid.main.util.enhancedGui.guiObjects.NotYetDialogueBox;
...
public class HotKeyListGui extends EnhancedGui {
	...
	EGuiButton edit, sortList, delete, toggleEnabled, loadKeyList, saveKeyList, resetKeyList;
	...
	public void initObjects() {
		...
		loadKeyList = new EGuiButton(this, startX + 20, endY + 3, 110, 20, "Reload HotKeys");
		saveKeyList = new EGuiButton(this, startX + 170, endY + 3, 110, 20, "Save HotKeys");
		resetKeyList = new EGuiButton(this, startX + 320, endY + 3, 110, 20, "Reset HotKeys");
		
		toggleEnabled.setVisible(false);
		
		keyList = new EGuiTextArea(this, endX - 198, startY + 20, 190, 220).setDrawLineNumbers(true);
		
		addObject(edit, delete, toggleEnabled, sortList, keyList, desc, loadKeyList, saveKeyList, resetKeyList);
	...
	public void drawObject(int mXIn, int mYIn, float ticks) {
		...
		//draw extra button container
		drawRect(startX, endY, endX, endY + 27, 0xff000000);
		drawRect(startX + 1, endY, endX - 1, endY + 26, 0xff1D1D1D);
	...
	public void actionPerformed(IEnhancedActionObject object) {
		...
		if (object.equals(delete)) {
			if (keyList.getCurrentLine() != null && keyList.getCurrentLine().getStoredObj() != null) {
				...
				if (key.isBuiltIn()) {
					...
					msgBox = new EGuiDialogueBox(guiInstance, wPos - 125, hPos - 48, 250, 75, DialogueBoxTypes.yesNo) {
						@Override
						public void actionPerformed(IEnhancedActionObject object) {
							if (object == yes) {
								this.close();
		...
		if (object.equals(sortList)) { addObject(new NotYetDialogueBox(this)); }
	...
	public void saveKeys() {
		mod.saveHotKeys();
		msgBox = new EGuiDialogueBox(guiInstance, wPos - 125, hPos - 48, 250, 75, DialogueBoxTypes.ok);
		msgBox.setDisplayString("Save");
		msgBox.setMessage("Success! Hotkeys saved.").setMessageColor(0x55ff55);
		guiInstance.addObject(msgBox);
	}
	
	public void resetKeys() {
		msgBox = new EGuiDialogueBox(guiInstance, wPos - 140, hPos - 58, 280, 95, DialogueBoxTypes.yesNo) {
			@Override
			public void actionPerformed(IEnhancedActionObject object) {
				if (object == yes) {
					this.close();
